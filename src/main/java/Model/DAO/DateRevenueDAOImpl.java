...
public class DateRevenueDAOImpl implements DateRevenueDAO{
	...
	public List<TotalReportModel> getDateRevenue(ReportQuery param){
		...
		case 0:
			query = String.format(queryFormat, "distinct new Model.MODEL.TotalReportModel(hour(a.id.dateOfRevenue), sum(a.price))",
												"date(a.id.dateOfRevenue) = date('" + param.getDate().toString() + "')",
												"hour(a.id.dateOfRevenue)",
												"hour(a.id.dateOfRevenue) asc");
			break;
		case 1:
			query = String.format(queryFormat, "distinct new Model.MODEL.TotalReportModel(dayname(a.id.dateOfRevenue), sum(a.price))",
												"weekofyear(a.id.dateOfRevenue) = weekofyear('" + param.getDate().toString() + "')",
												"dayname(a.id.dateOfRevenue)",
												"weekday(a.id.dateOfRevenue) asc");
			break;
			
		case 2:
			query = String.format(queryFormat, "distinct new Model.MODEL.TotalReportModel(day(a.id.dateOfRevenue), sum(a.price))",
												"month(a.id.dateOfRevenue) = month('" + param.getDate().toString() + "')",
												"day(a.id.dateOfRevenue)",
												"day(a.id.dateOfRevenue) asc");
			break;
			
		case 3:
			query = String.format(queryFormat, "distinct new Model.MODEL.TotalReportModel(month(a.id.dateOfRevenue), sum(a.price))",
												"quarter(a.id.dateOfRevenue) = '" + param.getQuarter() + "'",
												"month(a.id.dateOfRevenue)",
												"month(a.id.dateOfRevenue) asc");
			break;
			
		case 4:
			query = String.format(queryFormat, "distinct new Model.MODEL.TotalReportModel(month(a.id.dateOfRevenue), sum(a.price))",
												"year(a.id.dateOfRevenue) = '" + param.getYear() + "'",
												"month(a.id.dateOfRevenue)",
												"month(a.id.dateOfRevenue) asc");
			break;
